...
def gamma_h(epsilon, alpha):
    """
    Reparameterization for gamma rejection sampler without alpha boost
    """
    b = alpha - 1./3.
    c = 1./np.sqrt(9.*b)
    v = 1.+epsilon*c
...
def gamma_grad_h(epsilon, alpha):
    """
    Gradient of reparameterization without alpha boost
    """
    b = alpha - 1./3.
    c = 1./np.sqrt(9.*b)
    v = 1.+epsilon*c
...
def gamma_h_boosted(epsilon, u, alpha):
    """
    Reparameterization for gamma rejection sampler with alpha boost
    """
    B = u.shape[1]
    K = alpha.shape[0]
...
def gamma_grad_h_boosted(epsilon, u, alpha):
    """
    Gradient of reparameterization with alpha boost
    """
    B = u.shape[1]
    K = alpha.shape[0]
